			...
			for (int i = 0; i < newTopicSet.length; i++) {
				...
				if (!topicSeen) {
					...
					TimerTask tasknewA = new TRECScenarioRunnable(TRECSearcher.indexName,
							TRECSearcher.interestProfilePath + newTopicSet[i].topid + ".json", TRECSearcher.api_base
									+ "tweet/" + newTopicSet[i].topid + "/:tweetid/" + TRECSearcher.clientid,
							"A");
					...
					TimerTask tasknewB = new TRECScenarioRunnable(TRECSearcher.indexName,
							...
							TRECSearcher.api_base + "tweets/" + newTopicSet[i].topid + "/" + TRECSearcher.clientid,
							"A");
					LOG.info("Scenario B will start at epoch " + tomorrow.getTimeInMillis() + " Now is "
							+ now.getTimeInMillis());
					// [Deprecated!] Scenario A only for this year
					timerB.scheduleAtFixedRate(tasknewB,
							(long) (tomorrow.getTimeInMillis() - now.getTimeInMillis() + 1000),
